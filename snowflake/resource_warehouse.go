...
const (
	...
	whMulticlusterEnabled = "multicluster_enabled"
...
func resourceWarehouse() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			whMulticlusterEnabled: {
				Type:        schema.TypeBool,
				Optional:    true,
				Default:     false,
				ForceNew:    false,
				Description: "Specifies whether multicluster is enabled",
			},
			whMaxClusterCount: {
				Type:        schema.TypeInt,
				Optional:    true,
...
func createWarehouse(d *schema.ResourceData, meta interface{}) error {
	...
	fmt.Fprint(b, whName)
	fmt.Fprintf(b, " WITH ")
	// only add these parameters if multicluster is enabled
	if d.Get(whMulticlusterEnabled) == true {
		for _, attr := range []string{whMaxClusterCount, whMinClusterCount} {
...
func readWarehouse(d *schema.ResourceData, meta interface{}) error {
	...
	if meta.(*providerConfiguration).AccountType == "enterprise" {
		...
		err = db.QueryRow(stmtSQL).Scan(
			&name, &state, &instanceType, &size, &running, &queued,
			&isDefault, &isCurrent, &autoSuspend, &autoResume, &available, &provisioning, &quiescing, &other,
			&createdOn, &resumedOn, &updatedOn, &owner, &comment, &resourceMonitor,
			&actives, &pendings, &failed, &suspended, &uuid,
		)
...
func deleteWarehouse(d *schema.ResourceData, meta interface{}) error {
	...
	if _, err := db.Exec(sql); err != nil {
		return errwrap.Wrapf(fmt.Sprintf("Error dropping warehouse %q: {{err}}", whName), err)
	}
	return nil
}
